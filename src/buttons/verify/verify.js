...
const { guildProfileLink } = require('../../utils/discord')
  ...
  async execute(client, interaction) {
    ...
    if (VerifySettingDB.type === 'default') {
      ...
      let captchaVerify = new Embed(client, 'success')
        .setTitle('인증')
        .setDescription(`[여기](${web.baseurl}/verify?token=${token})로 접속하여 인증을 진행해주세요`)
      let captchaGuildEmbed = new Embed(client, 'info')
        captchaGuildEmbed.setThumbnail(guildProfileLink(interaction.guild))
        ...
        captchaGuildEmbed.setURL(`${web.baseurl}/verify?token=${token}`)
