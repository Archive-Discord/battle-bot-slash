...
export const playerStats = async (
  ...
  async function updateStats(pubgUser: PubgDB, mode: string, interaction: CommandInteraction) {
    ...
    if (mode === 'fpprank') {
      ...
      return interaction.editReply({
        embeds: [rankStatEmbed(squadFppStats, pubgUser.nickname, '1인칭 경쟁전', new Date())],
      });
    } else if (mode === 'tpprank') {
      const pubg = new PUBGClient({
        apiKey: config.pubgapikey,
      ...
      return interaction.editReply({
        embeds: [rankStatEmbed(squadTppStats, pubgUser.nickname, '3인칭 경쟁전', new Date())],
      });
    } else if (mode === 'tpp') {
      const pubg = new PUBGClient({
        apiKey: config.pubgapikey,
      ...
      return interaction.editReply({
        embeds: [statEmbed(squadTppStats, pubgUser.nickname, '3인칭 일반전', new Date())],
      });
    } else if (mode == 'fpp') {
      const pubg = new PUBGClient({
        apiKey: config.pubgapikey,
      ...
      return interaction.editReply({
        embeds: [statEmbed(squadFppStats, pubgUser.nickname, '1인칭 일반전', new Date())],
      });
    }
  }
};
