...
export const searchStocks = async (keyword: string): Promise<searchResults | undefined> => {
...
export const searchStockList = async (keyword: string): Promise<searchStocksList | undefined> => {
  if (searchStocksCache.has(keyword)) {
    return searchStocksCache.get(keyword);
  } else {
    const searchData = axios
      .get(
        `https://ac.stock.naver.com/ac?q=${encodeURI(keyword)}&target=stock,index,marketindicator`,
      )
      .then((res) => {
        return res.data;
      })
