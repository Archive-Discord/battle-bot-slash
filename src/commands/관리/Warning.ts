...
import Warning from "src/schemas/Warning";
 ...
 async execute(client, interaction) {
  ...
  if(subcommand === '지급') {
    ...
    const embedRemove = new MessageEmbed()
        ...
        .addField("유저", `<@${user?.id}>` + "(" + "`" + user?.id + "`" + ")", true)
    ...
    let insertRes = await Warning.find({userId: user?.id, guildId: interaction.guild?.id}).sort({published_date: -1}).limit(5).skip(warningID ? ((warningID - 1) * 5) : (0))
    let insertResLength = await Warning.find({userId: user?.id, guildId: interaction.guild?.id})
    let warns = new Array();
    ...
    const embedList = new MessageEmbed()
        .setColor('#ff7f00')
        .setTitle('경고')
        .setDescription(`${user?.username}님의 ${insertResLength.length}개의 경고중 최근 5개의 경고 기록입니다`)
