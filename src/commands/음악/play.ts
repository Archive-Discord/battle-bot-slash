...
export default new BaseCommand(
  ...
  async (client, message, args) => {
      ...
      let result = await client.player.search(song, {requestedBy: message.author}).catch((e) =>{}) as PlayerSearchResult
      ...
      if(result.playlist) {
        ...
        let msg = await message.reply({content: `<:playing:941212508784586772> **${userMention(message.author.id)} 노래를 선택해주세요!**`, components: [row]})
  ...
  },
  {
    ...
    async execute(client, interaction) {
      ...
      let result = await client.player.search(song, {requestedBy: interaction.user}).catch((e) =>{}) as PlayerSearchResult
      if(!result || !result.tracks.length) {
      ...
      if(result.playlist) {
        ...
        row.addComponents(select)
        interaction.editReply({content: `<:playing:941212508784586772> **${userMention(interaction.user.id)} 노래를 선택해주세요!**`, components: [row]})
