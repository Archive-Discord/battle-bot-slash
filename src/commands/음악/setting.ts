...
export default new BaseCommand(
  ...
  async (client, message, args) => {
    ...
    if (!db) {
      let musicChannel = await message.guild.channels.create(
        'battle-bot-music',
        { type: 'GUILD_TEXT' }
      )
      const row = new MessageActionRow().addComponents(buttonList)
      let msg = await musicChannel.send({
        embeds: [musicEmbed],
        components: [row]
      })
      ...
      return message.reply(
        `${channelMention(musicChannel.id)} 노래기능 설정이 완료되었습니다.`
      )
    } else {
      errembed.setDescription(
        `이미 ${channelMention(
          db.channel_id
        )}로 음악기능이 설정되있는거 같습니다. \n 채널을 삭제하셨거나 다시 설정을 원하시면 \`${
          config.bot.prefix
        }뮤직설정헤제\` 입력 후 다시 시도해주세요.`
      )
      return message.reply({ embeds: [errembed] })
    }
  },
  {
    ...
    async execute(client, interaction) {
      ...
      if (!db) {
        let musicChannel = await interaction.guild.channels.create(
          'battle-bot-music',
          { type: 'GUILD_TEXT' }
        )
        const row = new MessageActionRow().addComponents(buttonList)
