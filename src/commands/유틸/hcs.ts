...
import { searchSchool, login as HcsLogin, updateAgreement, registerSurvey } from 'hcs.js';
...
export default new BaseCommand(
  ...
  },
  {
      ...
      .addSubcommand((option) =>
        option
          ...
          .addStringOption((name) =>
            name.setName('이름').setDescription('이름을 적어주세요').setRequired(true),
          )
          .addStringOption((school) =>
            school.setName('학교').setDescription('학교이름을 적어주세요').setRequired(true),
          )
    ...
    async execute(client, interaction) {
      ...
      let errEmbed = new Embed(client, 'error').setTitle(`❌ 에러 발생`).setColor('#2f3136');
      ...
      if (subcommand === '설정') {
        ...
        let buttons = [
          new ButtonBuilder().setCustomId('hcs.ok').setLabel('동의').setStyle(ButtonStyle.Success),
          new ButtonBuilder().setCustomId('hcs.none').setLabel('거부').setStyle(ButtonStyle.Danger),
        ];
