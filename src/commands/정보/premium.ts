  ...
  async (client, message, args) => {
    ...
    if (!premium) {
      embed.setDescription(
        `이 서버는 프리미엄을 사용한 기록이 없습니다. [여기](${config.web?.baseurl}/premium) 에서 프리미엄을 구매해주세요.`,
      );
      return message.reply({ embeds: [embed] });
    } else {
      ...
      if (now > nextpay_date) {
        embed.setDescription(
          `이 서버는 프리미엄이 ${
            nextpay_date.getFullYear() +
            '년 ' +
            (nextpay_date.getMonth() + 1) +
            '월 ' +
            nextpay_date.getDate() +
            '일'
          }에 만료 되었습니다 [여기](${config.web?.baseurl}/premium) 에서 프리미엄을 구매해주세요.`,
        );
        return message.reply({ embeds: [embed] });
      } else {
        embed.setDescription(
