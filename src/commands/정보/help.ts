...
export default new BaseCommand({
  ...
  if (!args[0]) {
    client.categorys.forEach((category, command) => {
      ...
      embed.addField(`\`${config.bot.prefix}도움말 ${command}\``, `> ${command}관련 명령어들을 보내드려요!`)
    })
    ...
    commands.forEach((command) => {
      embed.addField(`\`${config.bot.prefix}${command.name}\``, `> ${command.description}`)
    })
    return message.reply({ embeds: [embed] })
  }
}, {
  ...
  async execute(client, interaction) {
    ...
    if (!interaction.options.getString('category')) {
        client.categorys.forEach((category, command) => {
          if(command === 'dev') return
          embed.setDescription(`아래에 있는 명령어들을 이용해 도움말을 보세요!`)
          embed.addField(`\`/도움말 ${command}\``, `> ${command}관련 명령어들을 보내드려요!`)
        })
