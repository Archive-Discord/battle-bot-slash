...
export default new BaseCommand(
  ...
  async (client, message, args) => {
    ...
    embed = new Embed(client, 'error').setDescription('상대방의 정보가 확인되지 않았어요ㅠㅠ\n상대방에게 먼저 \`!돈줘\`를 입력해 정보를 알려달라고 해주세요!')
      .setTimestamp()
    if (!tkdeoqkd) return m.edit({
      embeds: [embed]
    })
    const betting = parseInt(args[1])
    embed = new Embed(client, 'error').setDescription('사용법 : !송금 @멘션 (금액)')
      .setTimestamp()
    if (!betting) return m.edit({
      embeds: [embed]
    })

    embed = new Embed(client, 'error').setDescription('금액정보가 올바르지 않아요!\n특수문자가 들어가있다면 제거해주세요!(-)')
      .setTimestamp()
    if (message.content.includes("-")) return m.edit({
      embeds: [embed]
    })
    embed = new Embed(client, 'error').setDescription('송금할수 있는 최소 금액은 1000원부터 시작합니다!')
      .setTimestamp()
    if (betting < 1000) return m.edit({
      embeds: [embed]
    })
    ...
    embed = new Embed(client, 'error').setDescription('보유하고 있는 돈보다 많은 금액은 보낼수가 없어요.')
      .setTimestamp()
    if (money < betting) return m.edit({
      embeds: [embed]
    })
    embed = new Embed(client, 'error').setDescription('잠시만요 지금 누구한테 보내려는거죠?')
      .setTimestamp()
    if (message.author.id == user.id) return m.edit({
      embeds: [embed]
    })
    ...
    embed = new Embed(client, 'info').setTitle('돈을 보냈어요!')
